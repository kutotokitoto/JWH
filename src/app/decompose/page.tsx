...
const DraggableButton: React.FC<DraggableButtonProps> = ({ char, type, isUsed }) => {
  ...
  return (
    <button
      ref={drag}
      className={`${styles.draggableButton} ${isDragging ? styles.dragging : ''} ${isUsed ? styles.disabled : ''}`}
      disabled={isUsed}
    >
      {char}
    </button>
...
interface DropTargetProps {
  groupIndex: number;
  type: 'chosung' | 'jungsung' | 'jongsung';
  onDrop: (item: { char: string, type: string }, groupIndex: number) => void;
  chars: string[];
}

const DropTarget: React.FC<DropTargetProps> = ({ groupIndex, type, onDrop, chars }) => {
  const [{ isOver }, drop] = useDrop(() => ({
    accept: type,
    drop: (item: { char: string, type: string }) => onDrop(item, groupIndex),
    collect: (monitor) => ({
  ...
  return (
    <div
      ref={drop}
      className={`${styles.resultCell} ${isOver ? styles.dropOver : ''}`}
    >
      {chars.join(', ')}
    </div>
  );
...
// Adobe-KR-9 규격의 2780자 목록
const ADOBE_KR_9_CHARS = new Set([
  '가각간갇갈갉갊갋감갑값갓갔강갖갗같갚갛개객갠갣갤갬갭갯갰갱갸갹갼걀걋걍걔걘걜걥거걱건걷걸걺검겁겂것겄겅겆겉겊겋게겍겐겔겜겝겟겠겡겨격겪견겯결겷겸겹겻겼경겿곁계곈곌곕곗곘고곡곤곧골곪곬곯곰곱곳공곶곹과곽관괄괆괌괍괏괐광괒괘괜괠괢괩괬괭괴괵괸괼굄굅굇굉교굔굘굠굡굣굥구국군굳굴굵굶굻굼굽굿궁궂궃궈궉권궐궜궝궤궷궸귀귁귄귈귐귑귓귕규균귤귬귭그극근귿글긁긂긇금급긋긍긏긑긓긔긘긩기긱긴긷길긺김깁깃깄깅깆깊까깍깎깐깔깖깜깝깟깠깡깥깨깩깬깯깰깸깹깻깼깽꺄꺅꺆꺌꺍꺠꺤꺼꺽꺾껀껄껌껍껏껐껑껓껕께껙껜껨껫껭껴껸껼꼇꼈꼉꼍꼐꼬꼭꼰꼲꼳꼴꼼꼽꼿꽁꽂꽃꽅꽈꽉꽌꽐꽜꽝꽤꽥꽨꽸꽹꾀꾄꾈꾐꾑꾕꾜꾸꾹꾼꿀꿇꿈꿉꿋꿍꿎꿏꿔꿘꿜꿨꿩꿰꿱꿴꿸뀀뀁뀄뀌뀐뀔뀜뀝뀨뀰뀼끄끅끈끊끌끎끓끔끕끗끙끝끠끤끼끽낀낄낌낍낏낐낑나낙낚난낟날낡낢남납낫났낭낮낯낱낳내낵낸낻낼냄냅냇냈냉냐냑냔냗냘냠냡냣냥냬너넉넋넌넏널넑넒넓넘넙넛넜넝넢넣네넥넨넫넬넴넵넷넸넹넾녀녁년녇녈념녑녔녕녘녜녠녱노녹논놀놁놂놈놉놋농놑높놓놔놘놜놥놨놰뇄뇌뇍뇐뇔뇜뇝뇟뇡뇨뇩뇬뇰뇸뇹뇻뇽누눅눈눋눌눍눔눕눗눙눝눠눴눼뉘뉜뉠뉨뉩뉴뉵뉻뉼늄늅늉느늑는늗늘늙늚늠늡늣능늦늧늪늫늬늰늴늼늿닁니닉닌닏닐닒님닙닛닝닞닠닢다닥닦단닫달닭닮닯닳담답닷닸당닺닻닽닿대댁댄댈댐댑댓댔댕댖댜댠댱더덕덖던덛덜덞덟덤덥덧덩덫덮덯데덱덴델뎀뎁뎃뎄뎅뎌뎐뎔뎠뎡뎨뎬도독돈돋�
쳅쳇쳉쳊쳐쳔쳡쳤쳥쳬쳰촁초촉촌촐촘촙촛총촣촤촥촨촬촵촹쵀최쵠쵤쵬쵭쵯쵱쵸춈추축춘출춤춥춧충춰췄췌췍췐췔취췬췰췸췹췻췽츄츅츈츌츔츙츠측츤츨츩츰츱츳층츼치칙친칟칠칡칢침칩칫칬칭칮칰카칵칸칻칼캄캅캇캉캐캑캔캘캠캡캣캤캥캨캬캭캰컁컄커컥컨컫컬컴컵컷컸컹컽케켁켄켈켐켑켓켔켕켘켙켜켠켤켬켭켯켰켱켸코콕콘콛콜콤콥콧콩콰콱콴콸쾀쾃쾅쾌쾡쾨쾰쿄쿈쿠쿡쿤쿨쿰쿱쿳쿵쿼쿽퀀퀄퀌퀑퀘퀜퀠퀭퀴퀵퀸퀼큄큅큇큉큐큔큘큠크큭큰큲클큼큽킁킄킈키킥킨킬킴킵킷킸킹타탁탄탇탈탉탐탑탓탔탕태택탠탤탬탭탯탰탱탸턍턔터턱턴털턺턻텀텁텃텄텅테텍텐텔템텝텟텡텦텨텬텰텻텼톄톈토톡톤톧톨톰톱톳통톺톼퇀퇘퇴퇸퇻툇툉툐툥투툭툰툴툶툼툽툿퉁퉈퉜퉤퉷튀튁튄튈튐튑튕튜튠튤튬튱트특튼튿틀틂틈틉틋틍틑틔틘틜틤틥티틱틴틸팀팁팃팅파팍팎판팑팓팔팖팜팝팟팠팡팤팥패팩팬팯팰팸팹팻팼팽퍄퍅퍝퍼퍽펀펄펌펍펏펐펑펖페펙펜펠펨펩펫펭펴펵편펼폄폅폈평폐폔폘폡폣포폭폰폴폼폽폿퐁퐅퐈퐉퐝푀푄표푠푤푭푯푸푹푼푿풀풂품풉풋풍풔풩퓌퓐퓔퓜퓟퓨퓬퓰퓸퓻퓽프픈플픔픕픗픙픠픵피픽핀필핌핍핏핐핑하학한할핤핥함합핫항핰핳해핵핸핻핼햄햅햇했행햋햏햐햔햣향햬헀허헉헌헐헒헗험헙헛헝헠헡헣헤헥헨헬헴헵헷헸헹헿혀혁현혈혐협혓혔형혜혠혤혭호혹혼홀홅홈홉홋홍홑화확환활홥홧홨황홰홱홴횃횅회획횐횔횝횟횡효횬횰횹횻횽후훅훈훌훍훐훑훓훔훕훗훙훠훤훨훰훵훼훽휀휄휑휘휙휜휠휨휩휫휭휴휵휸휼흄흇흉흐흑흔흖흗흘흙흝흠흡흣흥흩희흰흴흼흽힁히힉힌힐힘힙힛힜힝힣'
]);

const isAdobeKR9Char = (cho: string, jung: string, jong: string | null) => {
  // 초성, 중성, 종성으로 한글 문자 생성
  ...
  // Adobe-KR-9 목록에 있는지 확인
  return ADOBE_KR_9_CHARS.has(char);
};
...
export default function Decompose() {
  ...
  // Adobe-KR-9 규격의 2780자에 해당하는 조합인지 확인하는 함수
  const isAdobeKR9Char = (cho: string, jung: string, jong: string | null) => {
    // Adobe-KR-9 규격에 따른 조합 규칙 구현
    // 여기서는 임시로 true를 반환
    // 실제 구현시에는 Adobe-KR-9 규격에 맞는 조합 규칙을 적용해야 함
    return true;
  };
  ...
  const handleDrop = (type: 'chosung' | 'jungsung' | 'jongsung') => 
    (item: { char: string, type: string }, groupIndex: number) => {
      if (type === 'chosung') {
        setChosungGroups(prev => {
          const newGroups = [...prev];
          if (!newGroups[groupIndex].includes(item.char)) {
            newGroups[groupIndex] = [...newGroups[groupIndex], item.char];
          }
          return newGroups;
        });
  ...
  const isCharUsed = (char: string, type: 'chosung' | 'jungsung' | 'jongsung') => {
    if (type === 'chosung') {
      return chosungGroups.some(group => group.includes(char));
    } else if (type === 'jungsung') {
      return jungsungGroups.some(group => group.includes(char));
    } else {
      return jongsungGroups.some(group => group.includes(char));
    }
  };
  ...
  const generateCombinations = () => {
    const combinations: CombinationCell[] = [];
          ...
          for (const cho of chosungGroups[i] || []) {
            for (const jung of jungsungGroups[j] || []) {
              for (const jong of jongsungGroups[k] || []) {
                ...
                if (combinationMode === 'adobe' && !isAdobeKR9Char(cho, jung, jong)) {
                  continue;
                }
              ...
              if (jongsungGroups[k].length === 0) {
                // Adobe-KR-9 모드일 때는 해당 규격에 맞는 문자만 포함
                if (combinationMode === 'adobe' && !isAdobeKR9Char(cho, jung, null)) {
                  continue;
                }
  ...
  const generateDownloadContent = () => {
    ...
    const allChars: HangulChar[] = [];
    
    // 모든 가능한 조합에 대해 한글 데이터 생성
    for (let i = 0; i < chosungGroups.length; i++) {
      for (let j = 0; j < jungsungGroups.length; j++) {
        for (let k = 0; k < jongsungGroups.length; k++) {
          for (const cho of chosungGroups[i] || []) {
            for (const jung of jungsungGroups[j] || []) {
              for (const jong of jongsungGroups[k] || []) {
                const unicode = 0xAC00 + 
                  ...
                  (JONGSUNG.indexOf(jong));
                const char = String.fromCharCode(unicode);
                allChars.push(generateHangulData(char, cho, jung, jong, i + 1, j + 1, k + 1));
              }
              ...
              if (jongsungGroups[k].length === 0) {
                const unicode = 0xAC00 + 
                  ...
                  (JUNGSUNG.indexOf(jung) * 28);
                const char = String.fromCharCode(unicode);
                allChars.push(generateHangulData(char, cho, jung, null, i + 1, j + 1, k + 1));
              }
            }
          }
        }
    ...
    return allChars
      ...
      .map(data => `${data.char}: ${data.roman}_${data.groupId} {${data.components.join(', ')}}`)
      .join('\n');
  };
    ...
    <DndProvider backend={HTML5Backend}>
      <div className={styles.container}>
                ...
                <div className={styles.buttons}>
                  {CHOSUNG.map((char) => (
                    <DraggableButton 
                      ...
                      char={char} 
                      type="chosung"
                      isUsed={isCharUsed(char, 'chosung')}
                    />
                  ))}
                </div>
              </div>
                ...
                <div className={styles.buttons}>
                  {JUNGSUNG.map((char) => (
                    <DraggableButton 
                      ...
                      char={char} 
                      type="jungsung"
                      isUsed={isCharUsed(char, 'jungsung')}
                    />
                  ))}
                </div>
              </div>
                ...
                <div className={styles.buttons}>
                  {JONGSUNG.filter(char => char !== '').map((char) => (
                    <DraggableButton 
                      ...
                      char={char} 
                      type="jongsung"
                      isUsed={isCharUsed(char, 'jongsung')}
                    />
                  ))}
                </div>
              </div>
              ...
              {charInfo ? (
                <>
                  ...
                  <div className={styles.componentList}>
                    <div>초성: {charInfo.components[0]}</div>
                    <div>중성: {charInfo.components[1]}</div>
                    <div>종성: {charInfo.components[2] || <span className={styles.noJongsung}>없음</span>}</div>
                  </div>
                </>
              ) : checkChar ? (
                <div className={styles.noInfo}>
                  해당 글자의 컴포넌트 정보를 찾을 수 없습니다.
                </div>
              ) : (
                <div className={styles.noInfo}>
                  한글 한 글자를 입력하세요
        ...
        <div className={styles.allComponentsSection}>
          ...
          <div className={styles.allComponentsList}>
